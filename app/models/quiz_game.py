...
class QuizGame:
    ...
    def run(self) -> None:
        try:
            ...
            '''
            KeyboardInterrupt:
                사용자가 터미널에서 Ctrl+C를 눌렀을 때 발생하는 예외입니다.

                예를 들어 프로그램이 input()으로 입력을 기다리는 중에
                사용자가 "아, 그냥 종료해야겠다" 하고 Ctrl+C를 누르면
                파이썬은 KeyboardInterrupt 예외를 발생시킵니다.

                이 예외를 처리하지 않으면 프로그램은 바로 멈추고,
                아래 같은 에러 메시지가 터미널에 그대로 출력될 수 있습니다.

                    KeyboardInterrupt

                이 프로젝트에서는 퀴즈 추가, 퀴즈 삭제, 점수 기록 같은 데이터가
                메모리(self.quizzes, self.best_score, self.game_history)에만 있다가
                종료 전에 state.json으로 저장됩니다.

                그래서 Ctrl+C로 갑자기 종료되더라도
                save_dict_data_to_json()을 호출해서 현재 상태를 저장한 뒤
                안전하게 종료하도록 처리합니다.


            EOFError:
                input()이 값을 더 이상 읽을 수 없을 때 발생하는 예외입니다.

                보통 콘솔에서 직접 실행할 때는 자주 만나지 않을 수 있지만,
                입력 스트림이 끊기거나, 테스트 환경에서 입력이 더 이상 없거나,
                터미널에서 Ctrl+D(macOS/Linux)를 누르는 경우 발생할 수 있습니다.

                예를 들어 input("번호 선택: ")이 실행되었는데
                사용자가 값을 입력하지 않고 입력 자체가 끝나버리면
                파이썬은 EOFError를 발생시킬 수 있습니다.

                EOF는 End Of File의 약자이고,
                "더 이상 읽을 입력이 없다"는 뜻으로 이해하면 됩니다.


            왜 둘을 같이 처리하는가:
                이 프로그램은 사용자의 input()에 많이 의존합니다.
                따라서 입력 도중 강제로 중단되는 상황이 생기면
                프로그램이 비정상 종료될 가능성이 있습니다.

                이때 데이터가 저장되지 않으면
                새로 추가한 퀴즈나 갱신된 점수, 게임 기록이 사라질 수 있습니다.

                그래서 KeyboardInterrupt와 EOFError를 잡아서
                현재 상태를 state.json에 저장하고 종료합니다.
            '''

            # Ctrl+C 또는 입력 스트림 종료가 발생해도 저장 후 안전하게 종료한다.
